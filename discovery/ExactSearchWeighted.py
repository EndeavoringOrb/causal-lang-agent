...
def bic_exact_search(
    ...
    suggested_graph : numpy.ndarray, shape=(d, d), optional
        A suggested adjacency matrix to guide the search. The score will be
        penalized for deviations from this graph. Default is None.
    suggestion_weight : float, optional
        The weight of the penalty for disagreeing with the suggested_graph.
        Higher values impose a stronger bias. Default is 1.0.
    ...
    # Validate suggested_graph if provided
    if suggested_graph is not None:
        assert suggested_graph.shape == (
            d,
            d,
        ), "Suggested graph must have shape (d, d)."
...
def generate_parent_graph(
    ...
    suggested_graph : numpy.ndarray, shape=(d, d), optional
        A suggested adjacency matrix to guide the search.
    suggestion_weight : float, optional
        The weight of the penalty for disagreeing with the suggested_graph.
    Returns
    -------
        ...
        if j == 0:
            ...
            for structure in it.combinations(parent_set, j):
                ...
                score = bic_score_node(
                    X, i, structure, suggested_graph, suggestion_weight
                )
...
def bic_score_node(X, i, structure, suggested_graph=None, suggestion_weight=1.0):
    ...
    optionally adding a penalty for deviations from a `suggested_graph`.
    """
    ...
    # MODIFICATION: Calculate and add the penalty if a suggested graph is provided.
    if suggested_graph is not None and suggestion_weight != 0:
        penalty = 0.0
        # Set of parents proposed by the current structure
        current_parents = set(structure)
        # Set of parents for node i from the suggested graph
        suggested_parents = set(np.where(suggested_graph[:, i] == 1)[0])

        # Calculate disagreements (Structural Hamming Distance for parents of i)
        # Edges in current structure but not suggested (False Positives)
        false_positives = len(current_parents - suggested_parents)
        # Edges suggested but not in current structure (False Negatives)
        false_negatives = len(suggested_parents - current_parents)

        penalty = (false_positives + false_negatives) * suggestion_weight

        return bic_value + penalty
    else:
        return bic_value
